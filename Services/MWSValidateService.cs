...
public class MWSValidateService : IMWSValidateService
{
    ...
    public async Task<Dictionary<string, string>> ValidateBlogCreateModel(BlogCreateEditViewModel model)
    {
        ...
        if (model.ImageFile is null) errorList.Add("ImageFile", "All blogs must have an image.");
    ...
    public async Task<Dictionary<string, string>> ValidatePostCreateModel(PostCreateViewModel model)
    {
        ...
        if (model.ImageFile == default!) errorList.Add("ImageFile", "All Posts must have an image.");

        if (model.ImageFile!.IsImage() == false)
            errorList.Add("ImageFile",
                "The file must be an image, please check your entry and try again.");

        if (await _postService.IsSlugUniqueAsync(model.Post.Slug, model.Post.BlogId) == false)
    ...
    {
        var errorList = new Dictionary<string, string>();
        if (model.ImageFile is not null && model.ImageFile.IsImage() == false)
            errorList.Add("ImageFile",
                "The file must be an image, please check your entry and try again.");
        ...
        IFormFileCollection files)
    {
        ...
        if (numberOfDifferences > 0) errorList.Add(string.Empty, "The form has an error; please refresh the page.");
