(function ($) {
  ...
  $('.po-btn .load-btn').on('click', function (e) {
    $(this)
      ...
      .queue(function (next) {
        next();
        const loadItemProjects = document.querySelectorAll('.load-item');
        removeLoadItemClass(loadItemProjects);
      })
      .delay(1)
      .queue(function (next) {
        next();
  ...
  function removeLoadItemClass(projectList) {
    projectList.forEach(function (project) {
      const projectClassListArr = Array.from(project.classList);
      if (projectClassListArr.includes('load-item')) {
        $(project).removeClass('load-item');
        $(project).addClass('portfolioProject');
      }
    });
  }
